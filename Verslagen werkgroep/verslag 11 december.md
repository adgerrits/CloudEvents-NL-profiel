...
Jan Jaap Zoutendijk ligt het gebruik van CloudEvents door het DSO toe.
...
Consent pagina een van de essenties van CloudEvents, die moet je er niet tussenuit halen. (dit sluit client credentials grant uit).
...
Hoe kan ik aanbieder afnemer intermediair(knooppunt) combineren in een CloudEvents profiel? Er is de behoefte om in APIs die via een knooppunt afgenomen worden afnemers achter het Knooppunt te kennen, maar ook om de identiteit van het Knooppunt toegang te geven tot APIs.
...
Mogelijk is dit via scopes in CloudEvents op te lossen.

Token formaat is een interessant onderwerp. CloudEvents zegt niets over token, Openid connect kent JWT tokens waar scopes( en identiteit) kunnen worden meegegeven.
...
6. Overgang CloudEvents SAML hoe deze twee standaarden combineren
8. Kadaster: Browser apllicaties die geen serverside code hebben is een interessante usecase (API keys zijn daar niet geschikt) misschien biedt CloudEvents profiel een alternatief. Achter grond is een behoefte aan het herkennen van individuele gebruikers t.b.v. monitoring e.d. Dynamic client registration mogelijk een oplossing voor deze case?
9. Logius: Vanuit Eid aan het kijken naar modernisering. CloudEvents/openid connect kandidaad. Met name voor BSN domein. Identiteit van de gebruiker inclusief machtiging (burger/ medewerker namens organisatie). Client gaat een rol spelen. 
